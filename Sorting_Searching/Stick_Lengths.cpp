...
void testcases(int testcase) {
    ...
    int n;
    cin>>n;
    v(int)arr(n); input(arr);

    // logic is to find median
    // because median is the element in which we can convert the whole array

    Sort(arr);
    cout<<findCost(arr[n/2],arr)<<endl;
}
